...
## Sub-agent Task
- `network_analyst` agent will pull all requests and response body that happened during the browser run. 
...
## Planning Output Contract (JSON Only)
{
  ...
  "subagent_tasks": {
    "network_analyst": {
      "task": "specific task for network evidence",
      "filter": {
        "resource_type": "xhr",
    ...
    "runtime_analyst": {
      "task": "specific task for console/runtime evidence",
      "filter": {
        "level": "error",
        "text_contains": "failed"
      }
    }
  }
}
