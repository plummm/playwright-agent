...
class NetworkAnalystNode:
    ...
    def __init__(
        ...
        *,
        ...
        self._max_parallel_prepare_calls = self._safe_int(
            role_cfg.get("max_parallel_prepare_calls"), default=5, min_value=1
        )
    ...
    async def run(
        ...
        *,
        ...
        per_request_results = analysis_result.get("items", [])
        ...
        return {
            "subagent_outputs": {"network_analyst": envelope},
    ...
    async def _prepare_records(self, *, run_state: Any, records: List[Dict[str, Any]], content_mode: str) -> List[Dict[str, Any]]:
        if not records:
            return []
        semaphore = asyncio.Semaphore(self._max_parallel_prepare_calls)

        async def _prepare_one(index: int, record: Dict[str, Any]) -> tuple[int, Dict[str, Any]]:
        ...
        tasks = [asyncio.create_task(_prepare_one(idx, rec)) for idx, rec in enumerate(records)]
    ...
    async def _analyze_each_record(
        ...
        *,
        ...
        async def _analyze_one(index: int, row: Dict[str, Any]) -> tuple[int, Dict[str, Any], Dict[str, Any]]:
            async with semaphore:
                ...
                return index, self._normalize_record_result(parsed_item=parsed_item, row=row), metrics

        tasks = [asyncio.create_task(_analyze_one(idx, row)) for idx, row in enumerate(prepared_records)]
